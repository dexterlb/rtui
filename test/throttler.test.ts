...
describe('Throttler.do', () => {
    ...
    it('calls the syncer when (and only when) the action throws', async () => {
        {
            ...
            let num_syncs = 0;
            let last_err = null;
            t.do(async () => { throw new Error('boom'); }, {
                sync_back: () => { num_syncs++; },
                err_handler: (err) => { last_err = err; },
            });
...

        {
            ...
            let num_syncs = 0;
            t.do(async () => {}, { sync_back: () => { num_syncs++; } });
    ...
    it('calls the syncer sync_back_after ms after a successful action', async () => {
        ...
        let num_syncs = 0;
        t.do(async () => {}, { sync_back: () => { num_syncs++; }, sync_back_after: 80 });
    ...
    it('cancels a pending sync-back when a new action arrives in the window', async () => {
        ...
        t.do(async () => {}, { sync_back: () => { a_syncs++; }, sync_back_after: 60 });
        await sleep(20);
        t.do(async () => {}, { sync_back: () => { b_syncs++; }, sync_back_after: 60 });
